...
import com.sdlproject.salesandrepair.adapters.AdapterCartItem;
import com.sdlproject.salesandrepair.adapters.AdapterProductSeller;
import com.sdlproject.salesandrepair.adapters.AdapterProductUser;
import com.sdlproject.salesandrepair.models.ModelCartItem;
...
public class ShopDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //each shop have its own products and orders so if user add items to cart and go back and open cart in
        // different shop then cart should be different, so delete cart data whenever user open this activity
        deleteCartData();
        cartCount();
                ...
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    ...
                    public void onSuccess(Void aVoid) {
                        ...
                        for(int i=0; i<cartItemList.size(); i++) {
                            ...
                            updateProductQuantity(pId, Integer.parseInt(quantity));
                        }
    ...
    private void updateProductQuantity(String pId, final int quantity) {

        final DatabaseReference reference = FirebaseDatabase.getInstance().getReference("Users").child(shopUid).child("Products").child(pId).child("productQuantity");
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        int productQuantity = Integer.parseInt("" + snapshot.getValue());
                        int newQuantity = productQuantity - quantity;
                        reference.setValue(""+newQuantity);
                    }
    ...
    private void sendFcmNotification(JSONObject notificationJo, final String orderId) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest("https://fcm.googleapis.com/fcm/send", notificationJo, new Response.Listener<JSONObject>() {
            ...
            public void onErrorResponse(VolleyError error) {
                //if sending fcm failed, still order details activity
                Intent intent = new Intent(ShopDetailsActivity.this, OrderDetailsUsersActivity.class);
                intent.putExtra("orderTo", shopUid);
                intent.putExtra("orderId", orderId);
                startActivity(intent);
